...
static const unsigned long sqlctx = 1086967459;
...
"select clase_docu ,nits.nit ,nvl(nits.digito,(-1)) ,sum((clientem.saldo+clie\
ntem.saldoc)) saldo2  from clientes ,clientem ,nits where (((((clientes.codigo\
_cliente=clientem.codigo_cliente and clientes.estado<>'R') and clientem.codigo\
_emp=:b0) and clientem.ano_mes=(select max(ano_mes)  from clientem cm where ((\
cm.codigo_emp=:b0 and cm.codigo_cliente=clientes.codigo_cliente) and cm.ano_me\
s<=:b2))) and clientes.codigo_nit=nits.codigo_nit) and (clientem.saldo<>0 or c\
lientem.saldoc<>0)) group by clase_docu,nits.nit,nits.digito order by saldo2  \
          ";
 static const char *sq0005 = 
"select n.nombre ,direccion ,((codigo_ciu||' ')||nombre_muni)  from nits n wh\
ere n.nit=:b0           ";
...
78,0,0,2,554,0,9,195,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
104,0,0,2,0,0,13,205,0,4,0,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
134,0,0,5,100,0,9,216,0,1,1,0,1,0,1,4,0,0,
152,0,0,5,0,0,13,217,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,
178,0,0,5,0,0,15,225,0,0,0,0,1,0,
192,0,0,2,0,0,15,243,0,0,0,0,1,0,
};
...

{
       ...
       sqlstm.sqlest = (unsigned char  *)&sqlca;
       sqlstm.sqlety = (unsigned short)0;
       sqlstm.sqhstv[0] = (unsigned char  *)&clase_docu;
