...
static const unsigned long sqlctx = 1314742330;
...
ientes.codigo_nit=nits.codigo_nit) and clientem.saldo<>0) and  not exists (sel\
ect 'x'  from clienser where clientes.codigo_cliente=codigo_cliente)) order by\
 clientes.codigo_cliente            ";
 static const char *sq0005 = 
"select to_char(fecha_vencmto,'yyyymmdd') ,nvl(facturam.saldo,0)  from factur\
as ,facturam where ((((((((facturas.tipo_factura=to_number(:b0) and facturas.t\
ipo_factura=facturam.tipo_factura) and facturas.tipo_documento=facturam.tipo_d\
ocumento) and facturas.numero_factura=facturam.numero_factura) and facturam.an\
o_mes=(select max(ano_mes)  from facturam fm where (((fm.tipo_factura=facturas\
.tipo_factura and fm.tipo_documento=facturas.tipo_documento) and fm.numero_fac\
tura=facturas.numero_factura) and fm.ano_mes<=to_number(:b1)))) and facturas.c\
odigo_cliente=:b2) and facturas.estado_factura=0) and facturam.saldo>0) and fa\
cturas.contado is null )           ";
...
{8,4114,
2,0,0,1,0,0,27,180,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,3,126,0,4,308,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
50,0,0,2,658,0,9,315,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
84,0,0,2,0,0,13,319,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,
110,0,0,2,0,0,15,361,0,0,0,0,1,0,
124,0,0,4,0,0,30,366,0,0,0,0,1,0,
138,0,0,5,657,0,9,441,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
164,0,0,5,0,0,13,444,0,2,0,0,1,0,2,9,0,0,2,4,0,0,
186,0,0,5,0,0,15,460,0,0,0,0,1,0,
200,0,0,6,54,0,4,684,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
