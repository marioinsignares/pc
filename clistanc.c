...
static const unsigned long sqlctx = 1314804207;
...
ntem ,nits where ((((((((clientes.codigo_cliente=clientem.codigo_cliente and c\
lientes.codigo_cliente>=:b0) and clientes.codigo_cliente<=:b1) and clientem.an\
o_mes=(select max(ano_mes)  from clientem cm where ((cm.codigo_emp=to_number(:\
b2) and cm.codigo_cliente=clientes.codigo_cliente) and cm.ano_mes<=to_number(:\
b3)))) and clientem.codigo_emp=to_number(:b2)) and clientes.estado<>'R') and c\
lientes.codigo_nit=nits.codigo_nit) and clientem.saldoc<>0) and  not exists (s\
elect 'x'  from clienser where clientes.codigo_cliente=codigo_cliente)) order \
by clientes.codigo_cliente            ";
 static const char *sq0005 = 
"select to_char(fecha_vencmto,'yyyymmdd') ,facturam.saldo ,facturas.numero_fa\
ctura ,decode(facturas.tipo_documento,'C','F',facturas.tipo_documento) ,fecha_\
documento ,codigo_nit  from facturas ,facturam where ((((((((facturas.tipo_fac\
tura=to_number(:b0) and facturas.tipo_factura=facturam.tipo_factura) and factu\
ras.tipo_documento=facturam.tipo_documento) and facturas.numero_factura=factur\
am.numero_factura) and facturas.contado='C') and facturas.codigo_cliente=:b1) \
and facturas.estado_factura=0) and facturam.saldo>0) and facturam.ano_mes=(sel\
ect max(ano_mes)  from facturam fm where ((((fm.tipo_factura=facturas.tipo_fac\
tura and fm.tipo_documento=facturas.tipo_documento) and fm.numero_factura=fact\
uras.numero_factura) and fm.ano_mes<=to_number(:b2)) and fm.ano_mes>=200209)))\
...
28,0,0,3,126,0,4,370,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
50,0,0,2,660,0,9,377,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
84,0,0,2,0,0,13,380,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,
110,0,0,2,0,0,13,404,0,3,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,
136,0,0,2,0,0,15,439,0,0,0,0,1,0,
150,0,0,4,0,0,30,444,0,0,0,0,1,0,
164,0,0,5,839,0,9,528,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
190,0,0,5,0,0,13,530,0,6,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,3,0,
0,
228,0,0,6,54,0,4,540,0,2,1,0,1,0,2,9,0,0,1,3,0,0,
250,0,0,5,0,0,13,571,0,5,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,9,0,0,2,9,0,0,
284,0,0,5,0,0,15,606,0,0,0,0,1,0,
298,0,0,7,54,0,4,717,0,2,1,0,1,0,2,9,0,0,1,3,0,0,
320,0,0,8,54,0,4,866,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...
/* VARCHAR codcli[10]; */ 
struct { unsigned short len; unsigned char arr[10]; } codcli;
...
it=:b1";
         ...
         if (con_lin > 62) {
             titulos();
             if (codigo_nit == 0)
                fprintf(fp,"%s%3s %-35s%s\n",enfatizado_on,codcli.arr,nomb_clie.arr,enfatizado_off);
             status = 0;
             con_lin++;
         }
         else {
