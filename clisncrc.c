...
static const unsigned long sqlctx = 1035560860;
...
re (((((codigo_emp=to_number(:b0) and tipo_documento='NC') and fecha>=to_date(\
:b1,'yyyymmdd')) and fecha<=to_date(:b2,'yyyymmdd')) and codigo_cliente>'0') a\
nd contado='C') order by numero            ";
...
{8,4114,
2,0,0,1,0,0,27,104,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,2,353,0,9,142,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
...
/*****************************************************************
* CLISTNCR.C Version 1.0 - Rev 1.2 - Oct. 9/92.
*
* clisting.c. Listado de La relacion de notas credito.
*
* Uso : clistncr codigo_emp fecha_inicial fecha_final device userid/password
*
...
int	status = 0;
	...
	printf ("Uso : clistncr codigo_emp fecha_ini fecha_fin\n");
...
void titulos()
{
 ...
 fprintf(fp,"%s%s%s%43s",diezcpi,condensado_on,ancho_on,"LISTADO DE NOTAS CREDITO - CONTADO");
 fprintf(fp,"%s%s%17s %d\n\n",ancho_off,diezcpi,"PAGINA ",con_pag);
 strorg(fechas,1,strlen(fechas));
 fprintf(fp,"%s%62s%s\n\n",enfatizado_on,fechas,enfatizado_off);
 fprintf(fp,"%s%s%11s%15s%-45s%19s%21s\n",condensado_on,"NOTA CREDITO","FECHA","CODIGO","     NOMBRE DEL CLIENTE","VALOR","DESCUENTO POR");
 fprintf(fp,"%12s%108s\n\n","NUMERO","CREDITO      RETENCION");
 con_lin = 8;
}
