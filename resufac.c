...
static const unsigned long sqlctx = 1300827632;
...
c ,f.cant_despachos ,f.valor_oftalmica ,f.valor_contacto ,f.valor_fletes ,f.va\
lor_iva ,f.valor_neto ,f.hojas ,f.rete_fte  from facturas f ,basura b ,control\
_fac cf where ((((((b.tipo_documento=f.tipo_documento and b.tipo_factura=f.tip\
o_factura) and b.numero_factura=f.numero_factura) and b.tipo_factura=cf.codigo\
_emp) and :b0=cf.codigo_emp) and to_number(to_char(f.fecha_documento,'yyyymmdd\
'))<=decode(to_number(:b1),1,to_number(to_char(to_date(((cf.ano||lpad(cf.mes_p\
roceso,2,0))||15),'yymmdd'),'yyyymmdd')),2,to_number(to_char(last_day(to_date(\
(cf.ano||lpad(cf.mes_proceso,2,0)),'yyyymm')),'yyyymmdd')))) and to_number(to_\
char(f.fecha_documento,'yyyymmdd'))>=to_number(to_char(to_date(decode(to_numbe\
r(:b1),1,to_number(((cf.ano||lpad(cf.mes_proceso,2,0))||lpad(1,2,0))),2,to_num\
ber(((cf.ano||lpad(cf.mes_proceso,2,0))||16))),'yymmdd'),'yyyymmdd'))) order b\
y codigo_cliente            ";
...
{8,4114,
2,0,0,1,0,0,27,145,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,2,0,0,15,222,0,0,0,0,1,0,
42,0,0,3,0,0,15,231,0,0,0,0,1,0,
56,0,0,4,0,0,30,233,0,0,0,0,1,0,
70,0,0,2,1029,0,9,256,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
100,0,0,2,0,0,13,259,0,12,0,0,1,0,2,4,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
162,0,0,2,0,0,15,303,0,0,0,0,1,0,
176,0,0,3,962,0,9,313,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
202,0,0,3,0,0,13,319,0,12,0,0,1,0,2,4,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
264,0,0,3,0,0,15,363,0,0,0,0,1,0,
278,0,0,5,111,0,4,372,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
300,0,0,6,54,0,4,450,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
322,0,0,7,542,0,4,555,0,6,1,0,1,0,2,3,0,0,2,3,0,0,2,3,0,0,2,9,0,0,2,9,0,0,1,9,
0,0,
360,0,0,8,120,0,4,574,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...
nes_c ,f.cant_despachos ,f.valor_oftalmica ,f.valor_contacto ,f.valor_fletes\
 ,f.valor_iva ,f.valor_neto ,f.hojas ,f.rete_fte  from facturas f ,control_f\
ac cf where (((((to_number(:b0)=f.tipo_factura and f.tipo_documento='F') and\
 to_number(:b0)=cf.codigo_emp) and to_number(to_char(f.fecha_documento,'yyyy\
mmdd'))<=decode(to_number(:b2),1,to_number(to_char(to_date(((cf.ano||lpad(cf\
.mes_proceso,2,0))||15),'yymmdd'),'yyyymmdd')),2,to_number(to_char(last_day(\
to_date((cf.ano||lpad(cf.mes_proceso,2,0)),'yyyymm')),'yyyymmdd')))) and to_\
number(to_char(f.fecha_documento,'yyyymmdd'))>=to_number(to_char(to_date(dec\
ode(to_number(:b2),1,to_number(((cf.ano||lpad(cf.mes_proceso,2,0))||lpad(1,2\
,0))),2,to_number(((cf.ano||lpad(cf.mes_proceso,2,0))||16))),'yymmdd'),'yyyy\
mmdd'))) and  not exists (select 'x'  from basura where ((f.tipo_factura=tip\
o_factura and f.tipo_documento=tipo_documento) and f.numero_factura=numero_f\
actura))) order by codigo_cl");
 sqlstm.stmt = sq0002;
 sqlstm.iters = (unsigned long  )1;
...

{
       ...
       sqlstm.sqhstv[11] = (unsigned char  *)&valor_rete_fte;
...
esas.codigo_nit=nits.codigo_nit and empresas.codigo_emp=:b1)";
 sqlstm.iters = (unsigned long  )1;
 sqlstm.offset = (unsigned short)360;
 sqlstm.selerr = (unsigned short)1;
 sqlstm.cud = sqlcud0;
 sqlstm.sqlest = (unsigned char  *)&sqlca;
 sqlstm.sqlety = (unsigned short)0;
