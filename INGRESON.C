...
static unsigned long sqlctx = 529563;
...
"select nvl(codigo_bco,' ') ,nvl(cheque,0) ,nvl(valor_chq,0) ,nvl(concepto,' \
') ,nvl(clase,' ') ,nvl(numero_factura,0) ,nvl(valor,0)  from ingresos_d where\
 (((codigo_emp=:b0 and ano_mes=:b1) and consecutivo=:b2) and libro=to_number(:\
b3))           ";
...
_mes) and d.consecutivo=c.consecutivo) and d.libro=to_number(:b0)) and c.codig\
o_emp=to_number(:b1)) and c.clase='RC') and c.ano_mes>=200601) and c.comproban\
te=to_number(:b2))           ";
...
{10,4114,0,0,0,
5,0,0,1,0,0,27,118,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,4,95,0,4,164,0,0,3,1,0,1,0,2,4,0,0,2,4,0,0,1,9,0,0,
63,0,0,5,65,0,4,173,0,0,2,1,0,1,0,2,4,0,0,1,9,0,0,
86,0,0,6,269,0,4,192,0,0,10,2,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,
9,0,0,2,4,0,0,2,4,0,0,1,9,0,0,1,9,0,0,
141,0,0,7,185,0,4,234,0,0,3,1,0,1,0,2,9,0,0,2,9,0,0,1,9,0,0,
168,0,0,2,247,0,9,246,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
199,0,0,3,339,0,9,251,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
226,0,0,2,0,0,15,258,0,0,0,0,0,1,0,
241,0,0,3,0,0,15,259,0,0,0,0,0,1,0,
256,0,0,8,82,0,5,264,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
279,0,0,2,0,0,15,274,0,0,0,0,0,1,0,
294,0,0,3,0,0,15,275,0,0,0,0,0,1,0,
309,0,0,9,0,0,30,277,0,0,0,0,0,1,0,
324,0,0,2,0,0,13,316,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,9,0,0,2,4,
0,0,2,4,0,0,
367,0,0,3,0,0,13,392,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,
};
...
struct { unsigned short len; unsigned char arr[50]; } comentarios;
...
struct { unsigned short len; unsigned char arr[80]; } nomb_clie;

/* VARCHAR nomb_ciu      [50]; */ 
struct { unsigned short len; unsigned char arr[50]; } nomb_ciu;

/* VARCHAR codigo_nita   [20]; */ 
struct { unsigned short len; unsigned char arr[20]; } codigo_nita;
...

{
         ...
         sqlstm.sqladtp = &sqladt;
         sqlstm.sqltdsp = &sqltds;
         sqlstm.stmt = "select fecha ,nvl(codigo_cliente,' ') ,nvl(nit,0) ,c\
odigo_nit ,nvl(valor_efectivos,0) ,nvl(comentarios,' ') ,ano_mes ,consecutivo \
into :b0,:b1,:b2,:b3,:b4,:b5,:b6,:b7  from ingresos_c where (((codigo_emp=:b8 \
and tipo_documento='RC') and ano_mes>=200601) and numero=:b9)";
         sqlstm.iters = (unsigned int  )1;
