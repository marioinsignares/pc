...
static const unsigned long sqlctx = 1061831017;
...
"select n.nombre ,n.nit ,nvl(n.digito,(-1)) ,sum(fs.saldo)  from facturas f ,\
facturam fs ,nits n where ((((((((n.codigo_nit=f.codigo_nit and fs.tipo_factur\
a=:b0) and fs.tipo_documento='C') and fs.numero_factura=f.numero_factura) and \
fs.tipo_factura=f.tipo_factura) and fs.tipo_documento=f.tipo_documento) and fs\
.ano_mes=(select max(ano_mes)  from facturam fm where (((fm.tipo_factura=f.tip\
o_factura and fm.tipo_documento=f.tipo_documento) and fm.numero_factura=f.nume\
ro_factura) and fm.ano_mes<=:b1))) and f.codigo_nit=n.codigo_nit) and fs.saldo\
<>0) group by n.nombre,n.nit,n.digito order by n.nombre            ";
...
28,0,0,2,0,0,15,173,0,0,0,0,1,0,
42,0,0,3,0,0,30,175,0,0,0,0,1,0,
56,0,0,4,115,0,4,186,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
78,0,0,2,611,0,9,195,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
100,0,0,2,0,0,13,200,0,4,0,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
130,0,0,2,0,0,15,225,0,0,0,0,1,0,
};
 ...
 **********************************************/
  ...
  sprintf(fecha_listado,"%s %d %s%s",mes[mes_corte-1],dia_corte,"de",r_t);
    ...
    /* EXEC SQL DECLARE C1 CURSOR FOR
             select n.nombre, n.nit, nvl(n.digito,-1), sum(fs.saldo)
             from   facturas f, facturam fs, nits n
             where  n.codigo_nit            = f.codigo_nit            and
                    fs.tipo_factura         = :codigo_emp             and
                    fs.tipo_documento       = 'C'                     and
                    fs.numero_factura       = f.numero_factura        and
                    fs.tipo_factura         = f.tipo_factura          and
                    fs.tipo_documento       = f.tipo_documento        and
                    fs.ano_mes =
                    (select max(ano_mes)
                     from  facturam fm
                     where fm.tipo_factura   = f.tipo_factura         and
                           fm.tipo_documento = f.tipo_documento       and
                           fm.numero_factura = f.numero_factura       and
                           fm.ano_mes       <= :ano_mes)              and
            f.codigo_nit              = n.codigo_nit                  and
            fs.saldo                 <> 0
            group by n.nombre, n.nit, n.digito
            order by n.nombre; */ 
