...
static const unsigned long sqlctx = 1238776430;
...
"select nombre ,nits.nit ,nvl(nits.digito,(-1)) ,clientem.saldo  from cliente\
s ,clientem ,nits where (((((clientes.codigo_cliente=clientem.codigo_cliente a\
nd clientes.estado<>'R') and clientem.codigo_emp=:b0) and clientem.ano_mes=(se\
lect max(ano_mes)  from clientem cm where ((cm.codigo_emp=:b0 and cm.codigo_cl\
iente=clientes.codigo_cliente) and cm.ano_mes<=:b2))) and clientes.codigo_nit=\
nits.codigo_nit) and clientem.saldo<>0) order by nits.nit            ";
...
lect max(ano_mes)  from clientem cm where ((cm.codigo_emp=:b0 and cm.codigo_cl\
iente=clientes.codigo_cliente) and cm.ano_mes<=:b2))) and clientes.codigo_nit=\
nits.codigo_nit) and clientem.saldo<>0) order by clientes.codigo_cliente      \
      ";
...
28,0,0,2,0,0,15,188,0,0,0,0,1,0,
42,0,0,3,0,0,15,191,0,0,0,0,1,0,
56,0,0,4,0,0,30,194,0,0,0,0,1,0,
70,0,0,5,115,0,4,205,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
92,0,0,2,457,0,9,216,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
118,0,0,3,472,0,9,219,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
144,0,0,2,0,0,13,226,0,4,0,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
174,0,0,3,0,0,13,229,0,4,0,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,2,4,0,0,
204,0,0,2,0,0,15,256,0,0,0,0,1,0,
218,0,0,3,0,0,15,259,0,0,0,0,1,0,
};
