...
static const unsigned long sqlctx = 1215695640;
...
216,0,0,4,0,0,15,289,0,0,0,0,1,0,
};
...

{
    ...
    sprintf(cadena,"%6.0d",tot_progresiv);
...
void titulos()
{
 ...
 if (digito > 0) {
     ...
     fprintf (fp,"-%s",fmtnum("z",cadena,r_t));
         ...
         fprintf (fp,"-%s","0");
 fprintf(fp,"\n%20s%s%s\n\n"," "," RELACION DE CUPONES PROGRESIVOS ",ancho_off);
 fecha_system(cadena);
