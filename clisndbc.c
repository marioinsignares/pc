...
static const unsigned long sqlctx = 1035560821;
...
"select numero_factura ,to_number(to_char(fecha_documento,'yyyymmdd')) ,codig\
o_cliente ,nvl(valor_neto,0)  from facturas where (((((tipo_factura=to_number(\
:b0) and tipo_documento='ND') and fecha_documento>=to_date(:b1,'yyyymmdd')) an\
d fecha_documento<=to_date(:b2,'yyyymmdd')) and contado='C') and estado_factur\
a=0) order by numero_factura            ";
...
28,0,0,2,350,0,9,137,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
54,0,0,3,115,0,4,154,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
76,0,0,2,0,0,13,167,0,4,0,0,1,0,2,4,0,0,2,4,0,0,2,9,0,0,2,4,0,0,
106,0,0,2,0,0,15,191,0,0,0,0,1,0,
120,0,0,4,0,0,30,192,0,0,0,0,1,0,
134,0,0,5,110,0,4,223,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...
odigo_nit=empresas.codigo_nit and empresas.codigo_emp=:b1)";
    sqlstm.iters = (unsigned long  )1;
...

{
           ...
           sqlstm.sqindv[2] = (         short *)0;
           sqlstm.sqharm[2] = (unsigned long )0;
           sqlstm.sqhstv[3] = (unsigned char  *)&valor_cartera;
...
void titulos()
{
 ...
 fprintf(fp,"%s%s%s%43s",diezcpi,condensado_on,ancho_on,"LISTADO DE NOTAS DEBITO - CONTADO");
...

{
 ...
 sqlstm.sqlvsn = 8;
 sqlstm.arrsiz = 4;
 sqlstm.stmt = "select nombre into :b0  from nits ,clientes where (nits.codi\
go_nit=clientes.codigo_nit and codigo_cliente=:b1)";
 sqlstm.iters = (unsigned long  )1;
