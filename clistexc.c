...
static const unsigned long sqlctx = 1152737172;
...
o>0) and f.contado='C') and fm.tipo_factura=f.tipo_factura) and fm.tipo_docume\
nto=f.tipo_documento) and fm.numero_factura=f.numero_factura) and fm.ano_mes=(\
select max(ano_mes)  from facturam where (((facturam.tipo_factura=:b0 and fact\
uram.tipo_documento=f.tipo_documento) and facturam.numero_factura=f.numero_fac\
tura) and facturam.ano_mes<=:b4))) order by fecha_documento            ";
...
28,0,0,6,70,0,4,345,0,2,1,0,1,0,2,3,0,0,1,9,0,0,
50,0,0,7,146,0,4,354,0,4,1,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,1,9,0,0,
80,0,0,2,402,0,9,365,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
102,0,0,2,0,0,13,370,0,6,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,3,0,
0,
140,0,0,8,254,0,4,387,0,5,4,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
174,0,0,3,693,0,9,404,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
208,0,0,4,281,0,9,406,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
238,0,0,3,0,0,15,425,0,0,0,0,1,0,
252,0,0,4,0,0,15,426,0,0,0,0,1,0,
266,0,0,2,0,0,15,430,0,0,0,0,1,0,
280,0,0,5,0,0,15,431,0,0,0,0,1,0,
294,0,0,9,0,0,30,433,0,0,0,0,1,0,
308,0,0,10,238,0,4,440,0,5,4,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
342,0,0,11,194,0,4,453,0,5,4,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
376,0,0,3,0,0,13,490,0,6,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,
0,
414,0,0,4,0,0,13,506,0,4,0,0,1,0,2,9,0,0,2,4,0,0,2,9,0,0,2,4,0,0,
444,0,0,12,509,0,4,522,0,5,4,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
478,0,0,5,138,0,9,1110,0,1,1,0,1,0,1,9,0,0,
496,0,0,5,0,0,13,1113,0,1,0,0,1,0,2,4,0,0,
514,0,0,5,0,0,15,1132,0,0,0,0,1,0,
};
...

{
        ...
        sqlstm.sqlvsn = 8;
        sqlstm.arrsiz = 6;
        sqlstm.stmt = "select cm.saldoc into :b0  from clientem cm where ((c\
m.codigo_emp=:b1 and cm.codigo_cliente=:b2) and cm.ano_mes=(select max(ano_mes\
)  from clientem where ((clientem.codigo_emp=:b1 and clientem.codigo_cliente=c\
m.codigo_cliente) and clientem.ano_mes<:b4)))";
        sqlstm.iters = (unsigned long  )1;
...
and fecha_documento<=to_date(:b4,'YYMMDD')) and contado='C') and estado_factur\
a=0) and valor_neto>0)";
 sqlstm.iters = (unsigned long  )1;
...
to_date(:b4,'YYMMDD')) and contado='C') and estado<>'2')";
 sqlstm.iters = (unsigned long  )1;
...
:b4=o.codigo_cliente) and nvl(o.estado,0)<>'R') and o.contado='C') and to_numb\
er(to_char(o.fecha_control,'yyyymmdd'))>decode(cf.numero_quincena,1,to_number(\
