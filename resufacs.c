...
static const unsigned long sqlctx = 1099922361;
...
"<=to_date(to_number(:b2),'yyyymmdd')) and codigo_cliente>=:b3) and codigo_cl\
iente<=:b4) and  not exists (select 'x'  from basura where ((f.tipo_factura=ti\
po_factura and f.tipo_documento=tipo_documento) and f.numero_factura=numero_fa\
ctura))) group by codigo_cliente order by codigo_cliente            ";
 static const char *sq0003 = 
"<=to_date(to_number(:b2),'yyyymmdd')) and codigo_cliente>=:b3) and codigo_cl\
iente<=:b4) and exists (select 'x'  from basura where ((f.tipo_factura=tipo_fa\
...
{8,4114,
2,0,0,1,0,0,27,138,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,4,0,0,30,245,0,0,0,0,1,0,
42,0,0,2,1324,0,9,266,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
76,0,0,2,0,0,13,269,0,14,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,
0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
146,0,0,2,0,0,15,319,0,0,0,0,1,0,
160,0,0,3,1319,0,9,328,0,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
194,0,0,3,0,0,13,334,0,14,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
264,0,0,3,0,0,15,384,0,0,0,0,1,0,
278,0,0,5,111,0,4,392,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
300,0,0,6,54,0,4,457,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
322,0,0,7,544,0,4,502,0,6,1,0,1,0,2,3,0,0,2,3,0,0,2,4,0,0,2,9,0,0,2,9,0,0,1,9,
0,0,
360,0,0,8,120,0,4,521,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...
r_neto,0)) \"12\" ,sum(valor_neto)  from facturas f where ((((((to_number(:b\
0)=tipo_factura and (tipo_documento='F' or tipo_documento='C')) and fecha_do\
cumento>=to_date(to_number(:b1),'yyyymmdd')) and fecha_documento");
 sqlstm.stmt = sq0002;
 sqlstm.iters = (unsigned long  )1;
 ...
 sqlstm.sqhstv[3] = (unsigned char  *)&clientei;
 sqlstm.sqhstl[3] = (unsigned long  )22;
 sqlstm.sqindv[3] = (         short *)0;
 sqlstm.sqharm[3] = (unsigned long )0;
 sqlstm.sqhstv[4] = (unsigned char  *)&clientef;
...
void titulos()
{
 ...
 if( serv == 0 )
   fprintf(fp,"%s%s%s%44s",diezcpi,condensado_on,ancho_on,"RESUMEN DE VENTAS CREDITO");
 else  {
 ...
 fprintf(fp,"%17s %d\n","PAGINA ",con_pag);
 fprintf(fp,"%41s%s%s\n","COMPARATIVO MENSUAL",ancho_off,diezcpi);
