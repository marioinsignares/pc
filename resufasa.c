...
static unsigned long sqlctx = 537075;
...
"mber(:b12)=tipo_factura and (tipo_documento='F' or tipo_documento='C')) and \
fecha_documento>=to_date(to_number(:b13),'yyyymmdd')) and fecha_documento<=to_\
date(to_number(:b14),'yyyymmdd')) and codigo_cliente>=:b15) and codigo_cliente\
<=:b16) and libro=to_number(:b17)) and  not exists (select 'x'  from basura wh\
ere ((f.tipo_factura=tipo_factura and f.tipo_documento=tipo_documento) and f.n\
umero_factura=numero_factura))) group by codigo_cliente order by codigo_client\
e            ";
...
fecha_documento>=to_date(to_number(:b13),'yyyymmdd')) and fecha_documento<=to_\
date(to_number(:b14),'yyyymmdd')) and codigo_cliente>=:b15) and codigo_cliente\
<=:b16) and libro=to_number(:b17)) and exists (select 'x'  from basura where (\
(f.tipo_factura=tipo_factura and f.tipo_documento=tipo_documento) and f.numero\
_factura=numero_factura))) group by codigo_cliente order by codigo_cliente    \
        ";
...
36,0,0,4,0,0,30,263,0,0,0,0,0,1,0,
51,0,0,2,1503,0,9,284,0,0,18,18,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,
1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,
138,0,0,2,0,0,13,287,0,0,14,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,
4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
209,0,0,2,0,0,15,337,0,0,0,0,0,1,0,
224,0,0,3,1498,0,9,346,0,0,18,18,0,1,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,
1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,1,9,0,0,1,9,0,0,1,9,0,0,
311,0,0,3,0,0,13,352,0,0,14,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,
4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
382,0,0,3,0,0,15,402,0,0,0,0,0,1,0,
397,0,0,5,111,0,4,410,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
420,0,0,6,54,0,4,467,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
443,0,0,7,544,0,4,512,0,0,6,1,0,1,0,2,3,0,0,2,3,0,0,2,4,0,0,2,9,0,0,2,9,0,0,1,
9,0,0,
482,0,0,8,120,0,4,531,0,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...
/* VARCHAR clientei[20]; */ 
struct { unsigned short len; unsigned char arr[20]; } clientei;

/* VARCHAR clientef[20]; */ 
struct { unsigned short len; unsigned char arr[20]; } clientef;
...
if (argc < 8)
   {
    ...
    printf ("Uso : resufac fecha_ini fecha_fin cliente_ini cliente_fin Libro device copias userid/password\n");
...
0)) \"12\" ,sum(valor_neto)  from facturas f where (((((((to_nu");
 sqlstm.stmt = sq0002;
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqhstv[15] = (         void  *)&clientei;
 ...
 sqlstm.sqhstv[16] = (         void  *)&clientef;
...
void titulos()
{
 ...
 if( serv == 0 )
   fprintf(fp,"%s%s%s%52s",diezcpi,condensado_on,ancho_on,"RESUMEN DE VENTAS (CREDITO+CONTADO)");
 else  {
 ...
 fprintf(fp,"%17s %d\n","PAGINA ",con_pag);
 fprintf(fp,"%44s%s%s\n","COMPARATIVO ANUAL",ancho_off,diezcpi);
