...
static unsigned long sqlctx = 268043;
...
"valor_danos,total_entradas_ano,total_salidas_ano,valor_entradas_ano,valor_sa\
lidas_ano,total_danos_ano,valor_danos_ano order by almacen.clase,categoria,alm\
acen.codigo_articulo            ";

 static const char *sq0005 = 
"select nombre  from tipos where (clase_material=:b0 and tipo_material=:b1)  \
         ";
...
{10,4114,0,0,0,
5,0,0,1,0,0,27,213,0,0,4,4,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,1,10,0,0,
36,0,0,3,138,0,4,287,0,0,3,0,0,1,0,2,9,0,0,2,4,0,0,2,3,0,0,
63,0,0,4,69,0,9,306,0,0,0,0,0,1,0,
78,0,0,4,0,0,13,310,0,0,3,0,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,
105,0,0,4,0,0,15,343,0,0,0,0,0,1,0,
120,0,0,2,1210,0,9,347,0,0,6,6,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,
159,0,0,2,0,0,13,351,0,0,20,0,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,
4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
254,0,0,2,0,0,13,456,0,0,20,0,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,
4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
349,0,0,2,0,0,15,515,0,0,0,0,0,1,0,
364,0,0,6,0,0,30,517,0,0,0,0,0,1,0,
379,0,0,5,85,0,9,630,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
402,0,0,5,0,0,13,632,0,0,1,0,0,1,0,2,9,0,0,
421,0,0,5,0,0,15,642,0,0,0,0,0,1,0,
436,0,0,7,351,0,4,806,0,0,4,3,0,1,0,2,4,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
};
...
/* VARCHAR clase2         [5]; */ 
struct { unsigned short len; unsigned char arr[5]; } clase2;

/* VARCHAR tipo2          [5]; */ 
struct { unsigned short len; unsigned char arr[5]; } tipo2;
...
/* VARCHAR nombre         [61]; */ 
struct { unsigned short len; unsigned char arr[61]; } nombre;

/* VARCHAR nombre2        [61]; */ 
struct { unsigned short len; unsigned char arr[61]; } nombre2;
...
/* VARCHAR clase_r        [10]; */ 
struct { unsigned short len; unsigned char arr[10]; } clase_r;
...
(existencia,0) ,nvl(ano_mes,0) ,nvl(costo_promedio,0) ,nvl(costo_reemplazo,0\
) ,nvl(total_entradas,0) ,nvl(total_salidas,0) ,nvl(valor_entradas,0) ,nvl(v\
alor_salidas,0) ,nvl(total_danos,0) ,nvl(valor_danos,0) ,nvl(total_entradas_\
ano,0) ,nvl(total_salidas_ano,0) ,nvl(valor_entradas_ano,0) ,nvl(valor_salid\
as_ano,0) ,nvl(total_danos_ano,0) ,nvl(valor_danos_ano,0)  from almacen ,alm\
acen_acu where ((((((almacen_acu.codigo_articulo(+)=almacen.codigo_articulo \
and almacen.estado='A') and (almacen.clase>=:b0 and almacen.clase<=:b1)) and\
 (almacen.categoria>=:b2 and almacen.categoria<=:b3)) and almacen_acu.clase(\
+)=almacen.clase) and ano_mes<=:b4) and ano_mes=(select max(ano_mes)  from a\
lmacen_acu au where (au.codigo_articulo(+)=almacen.codigo_articulo and ano_m\
es<=:b4))) group by almacen.clase,categoria,almacen.codigo_articulo,nombre,e\
xistencia,ano_mes,costo_promedio,costo_reemplazo,total_entradas,total_salida\
s,valor_entradas,valor_salidas,total_danos,");
    sqlstm.stmt = sq0002;
    sqlstm.iters = (unsigned int  )1;
...

{
    ...
    sqlstm.sqladtp = &sqladt;
    sqlstm.sqltdsp = &sqltds;
    sqlstm.stmt = "select sum((nvl(existencia,0)* nvl(costo_promedio,0))) in\
to :b0  from almacen al ,almacen_acu ac where (((((ac.codigo_articulo(+)=al.co\
digo_articulo and al.estado='A') and al.clase=:b1) and ac.clase(+)=al.clase) a\
nd ano_mes<=:b2) and ano_mes=(select max(ano_mes)  from almacen_acu au where (\
