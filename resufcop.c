...
static const unsigned long sqlctx = 1169152086;
...
lor_iva) ,sum(rete_fte) ,sum(valor_neto)  from facturas f where ((((to_number(\
:b0)=tipo_factura and tipo_documento='C') and fecha_documento>=to_date(to_numb\
er(:b1),'yyyymmdd')) and fecha_documento<=to_date(to_number(:b2),'yyyymmdd')) \
and  not exists (select 'x'  from basura where ((f.tipo_factura=tipo_factura a\
nd f.tipo_documento=tipo_documento) and f.numero_factura=numero_factura))) gro\
up by codigo_cliente order by codigo_cliente            ";
...
:b0)=tipo_factura and tipo_documento='C') and fecha_documento>=to_date(to_numb\
er(:b1),'yyyymmdd')) and fecha_documento<=to_date(to_number(:b2),'yyyymmdd')) \
and exists (select 'x'  from basura where ((f.tipo_factura=tipo_factura and f.\
...
{8,4114,
2,0,0,1,0,0,27,147,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,4,0,0,30,227,0,0,0,0,1,0,
42,0,0,2,600,0,9,248,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
68,0,0,2,0,0,13,251,0,10,0,0,1,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,4,0,0,2,4,0,
0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
122,0,0,2,0,0,15,292,0,0,0,0,1,0,
136,0,0,3,595,0,9,301,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
162,0,0,3,0,0,13,307,0,10,0,0,1,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,4,0,0,2,4,
0,0,2,4,0,0,2,4,0,0,2,4,0,0,2,4,0,0,
216,0,0,3,0,0,15,348,0,0,0,0,1,0,
230,0,0,5,111,0,4,356,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
252,0,0,6,54,0,4,431,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
274,0,0,7,105,0,4,534,0,4,1,0,1,0,2,3,0,0,2,3,0,0,2,4,0,0,1,9,0,0,
304,0,0,8,120,0,4,542,0,2,1,0,1,0,2,9,0,0,1,9,0,0,
};
...

{
 ...
 sqlstm.sqlvsn = 8;
 sqlstm.arrsiz = 10;
 sqlstm.stmt = "select cf.ano ,cf.mes_proceso ,cf.ptaje_iva into :b0,:b1,:b2\
  from control_fac cf where cf.codigo_emp=:b3";
 sqlstm.iters = (unsigned long  )1;
