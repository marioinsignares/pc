...
static const unsigned long sqlctx = 987018048;
...
factura ,facturas.tipo_documento ,(sysdate -facturas.fecha_documento) ,codigo_\
cliente  from facturas ,facturam where (((((((((facturas.tipo_factura=facturam\
.tipo_factura and facturas.tipo_documento=facturam.tipo_documento) and factura\
s.numero_factura=facturam.numero_factura) and facturam.ano_mes=(select max(fm.\
ano_mes)  from facturam fm where (((fm.tipo_factura=facturas.tipo_factura and \
fm.tipo_documento=facturas.tipo_documento) and fm.numero_factura=facturas.nume\
ro_factura) and fm.ano_mes<=to_number(:b0)))) and facturas.tipo_factura=to_num\
ber(:b1)) and facturas.codigo_cliente>=:b2) and facturas.codigo_cliente<=:b3) \
and facturas.estado_factura=0) and facturas.estado_cartera=0) and (facturas.fe\
cha_documento+59)<sysdate ) order by codigo_cliente,fecha_documento           \
 ";
...
{8,4114,
2,0,0,1,0,0,27,121,0,3,3,0,1,0,1,9,0,0,1,10,0,0,1,10,0,0,
28,0,0,2,857,0,9,162,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
...
178,0,0,2,0,0,15,238,0,0,0,0,1,0,
192,0,0,6,0,0,30,239,0,0,0,0,1,0,
206,0,0,2,0,0,13,272,0,6,0,0,1,0,2,9,0,0,2,4,0,0,2,4,0,0,2,9,0,0,2,4,0,0,2,9,0,
0,
};
